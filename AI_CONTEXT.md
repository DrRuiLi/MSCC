...
> Architectural map for AI assistants. Read this before exploring the codebase.
> Do **not** auto-update this file; propose changes and wait for explicit approval.
...
Slots: `molecule_info`, `sdf` (ChemmineR `SDF`), `igraph`, `isotopomer` (data.frame).

- Build via `get_Molecule_igraph_from_smiles()` / `get_Molecule_igraph_from_sdf()`.
- Graph attrs via generics `vdata` / `edata` (defined in `R/00_graph_primitives.R`).
- Isotopomer helpers: `Molecule_igraph_add_isotopomer()`, atom transfer via MCS or RXNMapper.
...
Typical entry: `get_CFM_data_from_smiles()` → `CFM_predict` / `CFM_annotate*` (Docker). Viewer: `shiny_vis_cfm()`.
...
| `Molecule_vis.R` | SDF↔igraph, `vis_sdf_igraph` |
| `dev_Chemmine.R` | SMILES↔SDF, MCS atom maps, `vis_smiles`, isotopologue formatting |
...
Access element table via `get_elem_table()` when schema harmonization is needed.

## Conventions agents must respect

1. **Isotope notation:** `[13]C`, `[2]H`, `[15]N`, `[18]O`, `[34]S` (not `13C` alone in chemform APIs).
2. **Adduct strings:** Prefer `[M+H]+` / `[M-H]-`; many APIs also accept `+`/`-` or polarity `0`/`1`.
3. **Polarity:** `0` = negative, `1` = positive (`get_polarity_from_adduct`, `get_polarity_suffix`).
...
- Vignettes: `vignettes/MSCC.Rmd` (stub), `vignettes/CFM_shiny.qmd` (CFM viewer usage).
...
- Treat this file as **read-only** unless the user explicitly asks to update it.
